...
public class AttributeSelection2 implements Serializable, RevisionHandler {
  ...
  //ricardo - public x private. Mudei a visibilidade deste atributo
  public int[] m_selectedAttributeSet;

/** the selected attributes */
  //ricardo - criei este novo atributo para armazenar as informações que precisamos dos atributos nominais
  public String[][] m_nameselectedAttributeSet;  
   ...
   */ 
    public String getAttributsCFS(int pk, String dataset) throws Exception{
        int i = 0;
  ...
  public AttributeSelection2() {
    ...
    //ricardo - inicializo já na construção, não estou preocupado em fazer isso dinâmico no momento
    m_nameselectedAttributeSet = new String[100][4];
   ...
   * 
   * @param ASEvaluator an evaluator obj
    ...
    if ((m_searchMethod instanceof RankedOutputSearch) && m_doRank == true) {
      ...
      for (int i = 0; i < m_numToSelect; i++) {
        m_selectionResults.append(Utils.doubleToString(
          ...
          + "\n");
        //ricardo - armazeno as informações quando estamos rodando um Ranker
        m_nameselectedAttributeSet[i][0]="Ranker";        
        m_nameselectedAttributeSet[i][1]= String.valueOf(m_attributeRanking[i][0] + 1);        
        m_nameselectedAttributeSet[i][2]= m_trainInstances.attribute((int) m_attributeRanking[i][0]).name();        
        m_nameselectedAttributeSet[i][3]= Utils.doubleToString(m_attributeRanking[i][1],3);        
      }
      ...
      for (int element : attributeSet) {
        m_selectionResults.append("                     "
          + m_trainInstances.attribute(element).name() + "\n");
        //ricardo - armazeno as informações quando estou rodando um cfs
        m_nameselectedAttributeSet[element][0]="CFS";
        m_nameselectedAttributeSet[element][1]=String.valueOf(element+1);
        m_nameselectedAttributeSet[element][2]=m_trainInstances.attribute(element).name();
      }
    }
  ...
  public static String SelectAttributes(ASEvaluation ASEvaluator,
    ...
    int classIndex = -1;
    boolean helpRequested = false;
    AttributeSelection2 trainSelector = new AttributeSelection2();
    ...
    try {
      ...
      if (searchName.length() != 0) {
        ...
        try {
          ...
          throw new Exception("Can't create Ranker obj");
        }
      }
